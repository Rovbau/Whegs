...
def match_scans(scan1, scan2):
    ...
    for _ in range(nsim):
        ...
        turn1 = np.math.degrees(math.atan2(R[1][0], R[0][0]))
        print(" Turn 1 Value : " + str(turn1))

        x, y = T
        return(round(x,2) ,round(y,2) ,round(turn,5))
...
if __name__ == '__main__':
    ...
    scan1 = lidar.get_lidar_data()
    #print(scan1[:10])
    scan1 = lidar.polar_to_cartesian(scan1)
    #print(scan1[:10])
    print("Move")
    #sleep(5)
    ...
    while loop:  
        ...
        scan2 = lidar.get_lidar_data()
        #print(scan2[:10])
        scan2 = lidar.polar_to_cartesian(scan2)
        #print(scan2[:10])
        ...
        #scan1 = [[3, 3], [8, 8]]
        #scan2 = [[6.1, 1.1], [12.9, 2.9]]

        x, y, pose = match_scans(scan1, scan2)
        print(x, y, pose)

        x_sum += x * math.cos(math.radians(pose_sum)) - y * math.sin(math.radians(pose_sum))
        y_sum += x * math.sin(math.radians(pose_sum)) + y * math.cos(math.radians(pose_sum))

        print("X: " + str(x_sum))
        print("Y: " + str(y_sum))

        pose_sum += pose
        print("Pose: " + str(pose_sum))
